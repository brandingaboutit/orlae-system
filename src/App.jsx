  ...
  return(
    <div>
      ...
      {modal&&<Modal title={editId?'Editar Lançamento':'Novo Lançamento'} onClose={()=>setModal(false)}>
        ...
        <Btn onClick={save}>{editId?'Atualizar':'Salvar Lançamento'}</Btn>
      </Modal>}
    </div>
  );
}
...
function Contas({contas,setContas}){
  ...
  const abrirNovo=()=>{setEditId(null);setF(EF);setModal(true);};
  const abrirEdicao=r=>{setEditId(r.id);setF({tipo:r.tipo,descricao:r.descricao,valor:String(r.valor),vencimento:r.vencimento,status:r.status,categoria:r.categoria||''});setModal(true);};
  const save=()=>{if(!f.descricao||!f.valor)return;const reg={...f,id:editId||uid(),valor:parseFloat(f.valor)};setContas(p=>editId?p.map(x=>x.id===editId?reg:x):[reg,...p]);setModal(false);};
  ...
  return(
    <div>
      <PH title="Contas a Pagar / Receber" action={<Btn onClick={abrirNovo}>+ Nova Conta</Btn>}/>
      ...
      <Card><Tbl cols={[
        ...
        {l:'Ação',k:'_',render:r=><div style={{display:'flex',gap:4}}>
          ...
          <ActBtns onEdit={()=>abrirEdicao(r)} onDelete={()=>setContas(p=>p.filter(x=>x.id!==r.id))}/>
        </div>},
      ]} rows={rows}/></Card>
      {modal&&<Modal title={editId?'Editar Conta':'Nova Conta'} onClose={()=>setModal(false)}>
...
function Compras({compras,setCompras,fornecedores}){
  ...
  const save=()=>{if(!f.fornecedor||!f.valor)return;const reg={...f,id:editId||uid(),valor:parseFloat(f.valor)};setCompras(p=>editId?p.map(x=>x.id===editId?reg:x):[reg,...p]);setModal(false);};
  return(
    <div>
      <PH title="Ordens de Compra" action={<Btn onClick={abrirNovo}>+ Nova Compra</Btn>}/>
      ...
      <Card><Tbl cols={[
        ...
        {l:'Ação',k:'_',render:r=><div style={{display:'flex',gap:4}}>
          ...
          <ActBtns onEdit={()=>abrirEdicao(r)} onDelete={()=>setCompras(p=>p.filter(x=>x.id!==r.id))}/>
        </div>},
...
function Clientes({clientes,setClientes}){
  ...
  return(
    <div>
      <PH title="Clientes" action={<Btn onClick={abrirNovo}>+ Novo Cliente</Btn>}/>
      <Card><Tbl cols={[{l:'Nome',k:'nome'},{l:'E-mail',k:'email'},{l:'Telefone',k:'telefone'},{l:'',k:'_',render:r=><ActBtns onEdit={()=>abrirEdicao(r)} onDelete={()=>setClientes(p=>p.filter(x=>x.id!==r.id))}/>}]} rows={clientes}/></Card>
      {modal&&<Modal title={editId?'Editar Cliente':'Novo Cliente'} onClose={()=>setModal(false)}>
        ...
        <Btn onClick={save}>{editId?'Atualizar':'Salvar Cliente'}</Btn>
      </Modal>}
    </div>
  );
}
...
function Vendas({vendas,setVendas,clientes,estoque,setEstoque,tipos,precificacoes,produtos,setFin}){
  ...
  return(
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:16}}>
        <SC title="Total Vendido" value={fmt(vendas.reduce((s,v)=>s+(v.valorTotal||0),0))} icon="💰" color={C.P}/>
        <SC title="Nº de Vendas" value={vendas.length} icon="🛍️" color={C.S}/>
        <SC title="Ticket Médio" value={fmt(vendas.length?vendas.reduce((s,v)=>s+(v.valorTotal||0),0)/vendas.length:0)} icon="📊" color="#0369a1"/>
      </div>
      <Card><Tbl cols={[
      ...
      {modal&&<Modal title={editId?'Editar Venda':'Nova Venda'} onClose={()=>setModal(false)} wide>
        ...
        <div style={{background:C.BG2,borderRadius:10,padding:14,marginBottom:14}}>
          <Grid cols="2fr 1.4fr 0.8fr auto">
            ...
            <DI label="SKU" value={itemAtual.sku} onChange={v=>setItemAtual(p=>({...p,sku:v}))} options={skuOptions} listId="vnd-sku" placeholder="Código (opcional)"/>
        ...
        <div style={{marginTop:16}}><Btn onClick={save}>{editId?'Atualizar Venda':'Registrar Venda'}</Btn></div>
      </Modal>}
    </div>
  );
}
...
function Estoque({estoque,setEstoque,tipos,produtos}){
  ...
  const abrirNovo=()=>{setEditId(null);setF(EF);setModal(true);};
  const abrirEdicao=r=>{setEditId(r.id);setF({sku:r.sku,nome:r.nome,tipoProduto:r.tipoProduto||'',cor:r.cor||'',tamanho:r.tamanho||'',quantidade:String(r.quantidade),custoTotal:String(r.custoTotal||''),frete:String(r.frete||'')});setModal(true);};
  const save=()=>{if(!f.sku||!f.nome)return;const item={...f,id:editId||uid(),quantidade:parseInt(f.quantidade||0),custoTotal:n(f.custoTotal),frete:n(f.frete),custoUnitario:custoUnit};setEstoque(p=>editId?p.map(x=>x.id===editId?item:x):[item,...p]);setModal(false);};
  const adj=(r,delta)=>{const q=parseInt(prompt(`${delta>0?'Adicionar':'Remover'} quantas unidades?`,'1')||0);if(q>0)setEstoque(p=>p.map(x=>x.id===r.id?{...x,quantidade:Math.max(0,x.quantidade+(delta>0?q:-q))}:x));};
  return(
    <div>
      <PH title="Estoque de Produtos Acabados" sub="Produtos prontos para venda" action={<Btn onClick={abrirNovo}>+ Novo Item</Btn>}/>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:16}}>
      ...
      <Card><Tbl cols={[
        ...
        {l:'Status',k:'_s',render:r=>{const s=stEst(r.quantidade);return<Bdg type={s.t}>{s.l}</Bdg>;}},
        {l:'Ações',k:'_',render:r=><div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
          <Btn sm onClick={()=>adj(r,1)} color="#166534">+ Entrada</Btn>
          <Btn sm onClick={()=>adj(r,-1)} color="#c47f17">− Saída</Btn>
          <ActBtns onEdit={()=>abrirEdicao(r)} onDelete={()=>setEstoque(p=>p.filter(x=>x.id!==r.id))}/>
        </div>},
      ]} rows={estoque}/></Card>
      {modal&&<Modal title={editId?'Editar Item':'Novo Item de Estoque'} onClose={()=>setModal(false)} wide>
        <Divider label="Identificação"/>
        <Grid><DI label="SKU *" value={f.sku} onChange={onSkuChange} options={produtos.map(p=>p.sku)} listId="est-sku" placeholder="EX: CAN-BRN-1"/><TI label="Nome *" value={f.nome} onChange={v=>setF(p=>({...p,nome:v}))}/></Grid>
        ...
        <Grid cols="1fr 1fr 1fr">
          ...
          <div style={{marginBottom:14}}><label style={lbl}>Custo Unitário (auto)</label><div style={{...inpRO,display:'flex',alignItems:'center',borderRadius:8,padding:'9px 12px',fontSize:13}}>{fmt(custoUnit)}</div></div>
        </Grid>
        <Btn onClick={save}>{editId?'Atualizar':'Salvar Item'}</Btn>
      </Modal>}
    </div>
  );
}
...
function CustoProduto({custos,setCustos,fornecedores,tipos,setTipos,produtos}){
  ...
  const onSkuChange=v=>{setF(p=>({...p,sku:v}));const prod=produtos.find(x=>x.sku===v);if(prod)setF(p=>({...p,sku:v,fornecedor:prod.fornecedor||p.fornecedor,tamanho:prod.tamanho||p.tamanho,tipoTecido:prod.material||p.tipoTecido}));};
  const abrirNovo=()=>{setEditId(null);setF(E);setModal(true);};
  const abrirEdicao=r=>{setEditId(r.id);const toS=v=>String(v||'');setF({...E,...r,custoTotal:toS(r.custoTotal),frete:toS(r.frete),quantidade:toS(r.quantidade),arteEstampa:toS(r.arteEstampa),etiqueta:toS(r.etiqueta),embalagem:toS(r.embalagem),tag:toS(r.tag),lacre:toS(r.lacre),cartao:toS(r.cartao),papelEmbrulho:toS(r.papelEmbrulho),adesivo:toS(r.adesivo),caixa:toS(r.caixa),sacola:toS(r.sacola),outros:toS(r.outros)});setModal(true);};
  const save=()=>{if(!f.tipoProduto)return;if(!tipos.includes(f.tipoProduto))setTipos(p=>[...p,f.tipoProduto]);const reg={...f,id:editId||uid(),custoTotal:n(f.custoTotal),frete:n(f.frete),quantidade:n(f.quantidade),custoUnitario:custoUnit,custoFinal:cuTot};setCustos(p=>editId?p.map(x=>x.id===editId?reg:x):[reg,...p]);setModal(false);};
  return(
    <div>
      <PH title="Custo de Produto" action={<Btn onClick={abrirNovo}>+ Nova Ficha</Btn>}/>
      <Card><Tbl cols={[
      ...
      {modal&&<Modal title={editId?'Editar Ficha':'Nova Ficha de Custo'} onClose={()=>setModal(false)} wide>
        <Divider label="Identificação"/>
        <Grid cols="1fr 1fr 1fr">
          ...
          <DI label="SKU" value={f.sku} onChange={onSkuChange} options={produtos.map(p=>p.sku)} listId="cp-sku" placeholder="EX: CAN-BRN-1"/>
        ...
        <Grid cols="1fr 1fr 1fr 1fr">
          <TI label="Custo Total (R$) *" value={f.custoTotal} onChange={v=>setF(p=>({...p,custoTotal:v}))} type="number" placeholder="0.00"/>
          <TI label="Frete (R$)" value={f.frete} onChange={v=>setF(p=>({...p,frete:v}))} type="number" placeholder="0.00"/>
        ...
        <F label="Observações"><textarea value={f.obs} onChange={e=>setF(p=>({...p,obs:e.target.value}))} placeholder="Informações relevantes..." style={{...inp,height:72,resize:'vertical'}}/></F>
        <Btn onClick={save}>{editId?'Atualizar Ficha':'Salvar Ficha'}</Btn>
      </Modal>}
    </div>
  );
}
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
          ...
          <div>
            ...
            <div style={{background:C.P,borderRadius:12,padding:20,color:'#fff',marginBottom:14}}>
              ...
              {[['Custo Base',fmt(custoBase)],['Markup (R$)',fmt(markupFinal)],['Preço de Venda',fmt(precoVenda)],['Lucro Bruto',fmt(lucroBruto)],['Margem',`${margem.toFixed(1)}%`]].map(([l,v])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,.15)'}}>
                  <span style={{opacity:.75,fontSize:13}}>{l}</span>
                  <span style={{fontWeight:900,fontSize:l==='Preço de Venda'?22:14}}>{v}</span>
                </div>
              ))}
            </div>
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <Card>
          ...
          <SI label="Modalidade" value={f.modalidade} onChange={v=>setF(p=>({...p,modalidade:v,pagamento:pgtosPorMod[v][0].v}))} options={[{v:'mesmo_dia',l:'Mesmo Dia'},{v:'um_dia',l:'1 Dia Útil'},{v:'sem_antecipacao',l:'Sem Antecipação (30+ dias)'}]}/>
          ...
          {f.valor&&<div style={{background:C.P,borderRadius:12,padding:18,color:'#fff',marginTop:8}}>
            {[['Taxa Aplicada',`${taxa}%`],['Valor Bruto',fmt(vBruto)],['Desconto',`- ${fmt(desconto)}`],['Valor Líquido',fmt(vLiq)]].map(([l,v])=>(
        ...
        <div>{Object.entries(TAXAS).map(([mod,taxas])=>(
...
function Fornecedores({fornecedores,setFornecedores}){
  ...
  return(
    <div>
      <PH title="Fornecedores" action={<Btn onClick={abrirNovo}>+ Novo Fornecedor</Btn>}/>
          ...
          <Card key={fo.id}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
              ...
              <div style={{display:'flex',gap:6}}>
                <button onClick={()=>abrirEdicao(fo)} style={{background:'none',border:'none',cursor:'pointer',color:C.S,fontSize:14}}>✎</button>
                <button onClick={()=>setFornecedores(p=>p.filter(x=>x.id!==fo.id))} style={{background:'none',border:'none',cursor:'pointer',color:'#ccc',fontSize:16}}>✕</button>
              </div>
            </div>
            <h4 style={{margin:'0 0 8px',color:C.P,fontWeight:800}}>{fo.nome}</h4>
            {fo.cnpj&&<p style={{margin:'3px 0',fontSize:12,color:'#888'}}>CNPJ: {fo.cnpj}</p>}
            {fo.email&&<p style={{margin:'3px 0',fontSize:12,color:'#888'}}>✉ {fo.email}</p>}
            {fo.telefone&&<p style={{margin:'3px 0',fontSize:12,color:'#888'}}>☎ {fo.telefone}</p>}
            {fo.endereco&&<p style={{margin:'3px 0',fontSize:12,color:'#888'}}>📍 {fo.endereco}</p>}
            {fo.site&&<a href={fo.site} target="_blank" rel="noreferrer" style={{fontSize:12,color:C.S,textDecoration:'none'}}>🌐 {fo.site}</a>}
          </Card>
        ))}
      </div>
      {modal&&<Modal title={editId?'Editar Fornecedor':'Novo Fornecedor'} onClose={()=>setModal(false)}>
        <TI label="Nome *" value={f.nome} onChange={v=>setF(p=>({...p,nome:v}))}/>
        <TI label="CNPJ" value={f.cnpj} onChange={v=>setF(p=>({...p,cnpj:v}))} placeholder="00.000.000/0001-00"/>
        <TI label="E-mail" value={f.email} onChange={v=>setF(p=>({...p,email:v}))} type="email"/>
        <TI label="Telefone" value={f.telefone} onChange={v=>setF(p=>({...p,telefone:v}))} placeholder="(21) 99999-9999"/>
        <TI label="Endereço" value={f.endereco} onChange={v=>setF(p=>({...p,endereco:v}))}/>
        <TI label="Site" value={f.site} onChange={v=>setF(p=>({...p,site:v}))} placeholder="https://"/>
        <Btn onClick={save}>{editId?'Atualizar':'Salvar Fornecedor'}</Btn>
      </Modal>}
    </div>
  );
}
...
function Materiais({materiais,setMateriais,fornecedores,tipos,setTipos}){
  ...
  return(
    <div>
      ...
      <Card><Tbl cols={[
        {l:'Data',k:'data'},{l:'Tipo',k:'tipoProduto'},{l:'Fornecedor',k:'fornecedor'},
        ...
        {l:'Obs',k:'obs',render:r=>r.obs?<span title={r.obs} style={{cursor:'help'}}>📝</span>:'-'},
        {l:'',k:'_',render:r=><ActBtns onEdit={()=>abrirEdicao(r)} onDelete={()=>setMateriais(p=>p.filter(x=>x.id!==r.id))}/>},
      ]} rows={[...materiais].sort((a,b)=>b.data?.localeCompare(a.data))}/></Card>
      {modal&&<Modal title={editId?'Editar Material':'Novo Material'} onClose={()=>setModal(false)}>
        ...
        <DI label="Fornecedor" value={f.fornecedor} onChange={v=>setF(p=>({...p,fornecedor:v}))} options={fornecedores.map(x=>x.nome)} listId="mat-forn" placeholder="Buscar fornecedor"/>
        <Grid><TI label="Quantidade *" value={f.quantidade} onChange={v=>setF(p=>({...p,quantidade:v}))} type="number"/><TI label="Custo Unitário (R$)" value={f.custoUnitario} onChange={v=>setF(p=>({...p,custoUnitario:v}))} type="number"/></Grid>
        ...
        <Btn onClick={save}>{editId?'Atualizar':'Salvar Material'}</Btn>
      </Modal>}
    </div>
  );
}
  ...
  return(
    <div>
      ...
      {(show('vendas')||show('vendas_diario'))&&<Card style={{marginBottom:12}}>
        ...
        <Tbl cols={[{l:'Pagamento',k:'p',render:r=>PGTO_LABELS[r.p]||r.p},{l:'Total',k:'t',render:r=>fmt(r.t)},{l:'%',k:'pp',render:r=>`${totV>0?((r.t/totV)*100).toFixed(1):0}%`}]} rows={Object.entries(byPgto).map(([p,t])=>({p,t,pp:p}))}/>
...
// ── EXPORTAR ──────────────────────────────────────────────────────────────────
function ExportModal({onClose,fin,contas,compras,vendas,clientes,estoque,custos,fornecedores,materiais,precificacoes,produtos}){
  const toCSV=(rows,cols)=>{if(!rows.length)return'Sem dados';return[cols.join(';'),...rows.map(r=>cols.map(c=>{const v=r[c]??'';return typeof v==='string'&&v.includes(';')?`"${v}"`:v;}).join(';'))].join('\n');};
  ...
  const sheets=[
    ...
    {l:'Precificação',fn:`orlae-precificacao-${todayStr()}.csv`,rows:precificacoes,cols:['tipoProduto','sku','custoBase','markup','markupValor','precoVenda','margem']},
  ...
  return(
    <Modal title="Exportar Dados" onClose={onClose}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
        {sheets.map(s=>(
          <button key={s.l} onClick={()=>download(toCSV(s.rows,s.cols),s.fn,'text/csv')} style={{background:'#fff',border:`1.5px solid ${C.BD}`,borderRadius:8,padding:'10px 12px',cursor:'pointer',fontFamily:'inherit',textAlign:'left'}}>
